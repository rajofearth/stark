...
interface ConvItem { id: string; icon: string; title: string; time: string; live?: boolean; }

const DEMO_CONVS: ConvItem[] = [
  { id: 'analytics-dashboard', icon: '◈', title: 'Analytics Dashboard', time: '2m ago',   live: true },
  { id: 'user-onboarding',     icon: '◎', title: 'User Onboarding Flow', time: '1h ago' },
  { id: 'bug-triage',          icon: '◉', title: 'Bug Triage Assistant',  time: '3h ago' },
  { id: 'report-gen',          icon: '▣', title: 'Report Generation',     time: 'Yesterday' },
  { id: 'data-sync',           icon: '⊞', title: 'Data Sync Pipeline',    time: '2d ago' },
];
...
function convItem(c: ConvItem): string {
  return `
  <div class="conv-item${c.id === 'analytics-dashboard' ? ' active' : ''}" data-conv="${c.id}">
    <div class="conv-icon">${c.icon}</div>
    <div class="conv-info">
      <div class="conv-title">${c.title}</div>
      <div class="conv-time">${c.time}</div>
    </div>
    ${c.live ? `<div class="conv-live"></div>` : ''}
  </div>`;
}
